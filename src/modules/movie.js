...
const getters = {
  [types.getters.movies]: state => {
    ...
    if (state.query.search) {
      movies = movies.filter(movie => movie.movie.name.toLowerCase().includes(state.query.search))
    }

    if (state.query.rows) {
      movies = movies.filter(movie => movie.room.rows >= state.query.rows)
    }

    if (state.query.seats) {
      movies = movies.filter(movie => movie.room.seats >= state.query.seats)
    }
...
const mutations = {
  ...
  [types.mutations.setRows]: (state, rows) => {
